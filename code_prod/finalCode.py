...
for tweetDate in dataDates:    
    ...
    input_path = '/mountTeamEvans/' + tweetDate + ' Coronavirus Tweets.CSV'
    tweet_raw = spark.read.option("header","true").option("mode","DROPMALFORMED").csv(input_bucket + input_path)
    #display(tweet_raw)
    ...
    wordcloud.to_file(tweetDate + '-wordcloud.png')
    ...
    def plot12():
        ...
        pdf.plot.line(x="Language", y="Total_count", title="Top10 Languages used by Tweets")
    ...
    coronadf = (spark.read.format("csv").options(header="true").load(input_bucket+input_path))
    coronadf.show()
